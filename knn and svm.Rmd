...
library(class)
library(ggplot2)
data <- read.csv("c:/NYU BIOSTATICTICS/ML/output_file1.csv")
data=data[,-1]
set.seed(123) 
...
for (k in 1:10) {
  ...
  train_accuracy_seq[k] <- sum(predicted_train == train_data$Diagnosis) / length(predicted_train)
  
  # Predict using test data
  predicted_test <- knn(train = train_data[, -1], test = test_data[, -1], 
                        cl = train_data$Diagnosis, k = k)
  test_accuracy_seq[k] <- sum(predicted_test == test_data$Diagnosis) / length(predicted_test)
}
